...
def get_argued_on(text):

    text = text.lower()

    pattern1 = r"(?:argued on|argument|supported on|inquiry on)\s*[:]\s*(\d{1,4}[-/.]\d{1,2}[-/.]\d{1,4})"
    pattern2 = r"(?:argued on|argument|supported on|inquiry on)\s*[:]\s*(\d{1,2}.*?\d{4})"
...
def main(dir_path):
    ...
    for file in list_of_pdfs:
        try:
            ...
            judgement_dates.extend(get_argued_on(text))
        except Exception as e:
            print(f"Error processing file {file}: {e}")
            continue

    # Write judge names to CSV
    with open("argue.csv", "w", newline="") as csvfile:
